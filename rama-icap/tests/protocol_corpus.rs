use rama_icap::codec::{
    ...
    parse_request_head_with_config, parse_response_head,
};
use rama_icap::proto::{EncapsulatedKind, Method, StatusCode};
...
fn applies_icap_composition_errata() {
    ...
    assert_eq!(
        parse_response_head(Method::Respmod, body_on_204, &mut headers),
        Err(ParseError::InvalidComposition)
    );
}
...
fn parses_rfc_and_errata_response_head_corpus() {
    // RFC 3507 section 4.10.3, Preview, the errata's bodyless responses,
    // and the partial-content extension's 206 response.
    let corpus: &[(&[u8], Method<'_>, StatusCode)] = &[
        (
            b"ICAP/1.0 200 OK\r\n\
              Methods: RESPMOD\r\n\
