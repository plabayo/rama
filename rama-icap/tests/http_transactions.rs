...
use rama_http_types::{Body, Request as HttpRequest, body::Frame};
use rama_icap::{
    client::ClientConnection,
    codec::{Header, RequestLine, ResponseLine},
    http::{ClientRequest, Encapsulated},
    io::BodyEnd,
...
fn response(method: MethodKind, status: StatusCode, parts: Option<EncapsulatedParts>) -> Response {
    ...
    let istag = matches!(status, StatusCode::OK | StatusCode::PARTIAL_CONTENT).then_some(istag);
    let headers = istag.as_slice();
    Response::new(
        method,
        ResponseLine::new(
            status,
...
async fn typed_client_streams_a_body_bearing_early_response() {
    ...
    let client = async move {
        ...
        let request = HttpRequest::builder()
            ...
            .unwrap();
        let request =
            ClientRequest::reqmod(request_line(Method::Reqmod, "early"), &[], request, None)
                .unwrap();
        let mut connection = ClientConnection::new(ServiceInput::new(client_io));
...
async fn typed_client_rejects_unselected_206_original_offset() {
    async fn probe(offset: u64, with_trailers: bool) {
        ...
        let peer = async move {
            ...
            while !request.windows(7).any(|window| window == b"\r\n0\r\n\r\n") {
                let mut buf = [0; 512];
