...
type TrailersSender = oneshot::Sender<Header<'static>>;
...
enum Kind {
    ...
    Chan {
        ...
        trailers_rx: oneshot::Receiver<Header>,
    },
}
...
impl Incoming {
    ...
    pub(crate) fn new_channel(content_length: DecodedLength, wanter: bool) -> (Sender, Incoming) {
        ...
        let (trailers_tx, trailers_rx) = oneshot::channel(); // TODO: check
...
impl Sender {
    ...
    #[allow(unused)]
    pub(crate) async fn send_trailers(&mut self, trailers: Header) -> crate::Result<()> {
