...
impl<B> StreamingBody for Limited<B>
where
    B: StreamingBody,
...
mod tests {
    use super::*;
    use crate::body::util::{BodyExt, Full, StreamBody};
    use bytes::Bytes;
    ...
    fn body_from_iter<I>(into_iter: I) -> impl StreamingBody<Data = Bytes, Error = Infallible>
    where
        I: IntoIterator,
    ...
    impl StreamingBody for SomeTrailers {
        type Data = Bytes;
        ...
        fn poll_frame(
            ...
            Poll::Ready(Some(Ok(Frame::trailers(crate::header::HeaderMap::new()))))
        }
    }
    ...
    impl StreamingBody for ErrorBody {
        type Data = &'static [u8];
