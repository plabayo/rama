...
impl<T: StreamingBody + Unpin + ?Sized> StreamingBody for &mut T {
    type Data = T::Data;
    type Error = T::Error;
...
impl<P> StreamingBody for Pin<P>
where
    P: Unpin + ops::DerefMut,
    P::Target: StreamingBody,
{
    type Data = <<P as ops::Deref>::Target as StreamingBody>::Data;
    type Error = <<P as ops::Deref>::Target as StreamingBody>::Error;
...
impl<T: StreamingBody + Unpin + ?Sized> StreamingBody for Box<T> {
    type Data = T::Data;
    type Error = T::Error;
...
impl<B: StreamingBody> StreamingBody for http::Request<B> {
    type Data = B::Data;
    type Error = B::Error;
...
impl<B: StreamingBody> StreamingBody for http::Response<B> {
    type Data = B::Data;
    type Error = B::Error;
...
fn _assert_bounds() {
    fn can_be_trait_object(
        _: &dyn StreamingBody<Data = std::io::Cursor<Vec<u8>>, Error = std::io::Error>,
    ) {
    }
}
