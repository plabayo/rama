...
//! # Expected output
//!
//! The server will start and listen on `:53800`. You can use `curl` to interact with the service:
//!
//! ```sh
//! curl -v https://127.0.0.1:52800
//! curl -k -v https://127.0.0.1:53800
//! ```
//!
...
async fn main() {
    ...
    shutdown.spawn_task_fn(|guard| async move {
        ...
        let tcp_service = ServiceBuilder::new()
            ...
            .service(Forwarder::target("127.0.0.1:52800".parse().unwrap()));

        TcpListener::bind("127.0.0.1:53800")
            .await
    ...
    shutdown.spawn_task_fn(|guard| async {
        TcpListener::bind("127.0.0.1:52800")
            .await
