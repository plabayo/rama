...
mod tests {
    ...
    use rama_udp::{
        DatagramCapabilities, DatagramError, DatagramSender, DatagramSocket, SendDatagram,
    };
    ...
    fn receive_discards_truncated_groups_and_splits_complete_groups() {
        ...
        .unwrap();
        let mut endpoint =
            proto::Endpoint::new(Arc::new(EndpointConfig::default()), None, false, None);
    ...
    fn stateless_response_uses_actual_local_address_not_original_destination() {
        ...
        let server = ServerConfig::with_single_cert(
            vec![cert.cert.into()],
            rama_crypto::pki_types::PrivatePkcs8KeyDer::from(cert.signing_key.serialize_der())
                .into(),
        )
        .unwrap();
    ...
    async fn a_socket_retired_in_a_failing_poll_is_dropped_outside_the_endpoint_lock() {
        ...
        let mut cx = Context::from_waker(Waker::noop());
        // The receive pass visits the retiring socket first (it fails and retires), then the
        // active one (fatal).
        assert!(matches!(
    ...
    async fn a_shared_receive_allowance_rotates_across_hot_sockets_without_starving_any() {
        ...
        let normal = |n: usize| -> VecDeque<_> {
            (0..n).map(|_| (vec![0u8; 6], metadata(6, None))).collect()
        };
...
mod lifecycle_tests {
    use super::tests::{Captured, TestSocket, pin_active_socket, release_lease};
    use super::*;
    ...
    async fn saturated_receive_queue_drops_and_the_transfer_still_completes() {
        ...
        let mut stream = tokio::time::timeout(Duration::from_secs(5), client_conn.accept_uni())
            ...
            .unwrap();
        let received =
            tokio::time::timeout(Duration::from_secs(10), stream.read_to_end(payload.len()))
                .await
                .unwrap()
                .unwrap();
    ...
    async fn bulk_transmit_yields_at_the_quota_and_lets_another_connection_progress() {
        ...
        let mut stream = tokio::time::timeout(Duration::from_secs(5), bulk_server.accept_uni())
            ...
            .unwrap();
        let received =
            tokio::time::timeout(Duration::from_secs(20), stream.read_to_end(payload.len()))
                .await
                .unwrap()
                .unwrap();
    ...
    async fn receive_queue_limits_accept_the_exact_boundary() {
        let payload =
            usize::try_from(EndpointConfig::default().get_max_udp_payload_size()).unwrap();
    ...
    mod dial9_tests {
        ...
        use rama_core::telemetry::dial9::{
            Dial9Handle, Dial9HandleTokioExt as _, DiskBuffer, TokioAttachOptions,
            ...
            if let Some(size) = datagram.segment_size() {
                ...
                if datagram.segment_count() < 3 {
                    ...
                    while offset < payload.len() {
                        ...
                        let mut one = rama_udp::SendDatagram::new(
