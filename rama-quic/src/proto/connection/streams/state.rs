...
impl StreamRecv {
    ...
    pub(super) fn free(self, initial_max_data: u64) -> Self {
        match self {
            #[expect(
                clippy::unreachable,
                reason = "`free` is only called on `Open`/`ResetClosed` entries taken out of `recv`"
            )]
...
pub struct StreamsState {
    ...
    /// whose allocation has not been released yet
    ...
    /// Application opens still waiting for capacity, independent of frame transmission.
    pub(super) blocked_open: [bool; 2],
}
...
impl StreamsState {
    ...
    pub(super) fn resource_usage(&self) -> super::StreamResourceUsage {
        super::StreamResourceUsage {
            ...
            unacknowledged_bytes: self.buffered_data,
    ...
    pub(crate) fn set_params(&mut self, params: &TransportParameters) {
        ...
        // A server may expose an early-data handle before a fragmented ClientHello has
        // delivered its transport parameters. Wake opens that previously saw a zero limit.
        for dir in Dir::iter() {
            let index = dir as usize;
            if self.blocked_open[index] && self.next[index] < self.max[index] {
                self.blocked_open[index] = false;
                self.streams_blocked[index] = false;
    ...
    pub(crate) fn zero_rtt_rejected(&mut self) {
        ...
        for dir in Dir::iter() {
            for i in 0..self.next[dir as usize] {
                ...
                #[expect(
                    clippy::unwrap_used,
                    reason = "every locally initiated id below `next` was inserted by `open` and is only removed here when 0-RTT is rejected"
                )]
                self.send.remove(&id).unwrap();
                if dir == Dir::Bi {
                    #[expect(
                        clippy::unwrap_used,
                        reason = "bidirectional streams opened locally always get a `recv` entry in `insert`"
                    )]
    ...
    pub(crate) fn received(
        ...
        if closed {
            #[expect(
                clippy::unwrap_used,
                reason = "`closed`/`stopped` were just read from this occupied `recv` entry"
            )]
    ...
    pub fn received_reset(
        ...
        let end = rs.end;
        // A stopped stream returned its credits up to `end` when it was stopped.
    ...
    pub(crate) fn reset_acked(&mut self, id: StreamId) {
        match self.send.entry(id) {
            ...
            hash_map::Entry::Occupied(e) => {
                if e.get().as_ref().map(|s| s.state) == Some(SendState::ResetSent) {
                    e.remove_entry();
    ...
    pub(in crate::proto::connection) fn write_control_frames(
        &mut self,
        buf: &mut Vec<u8>,
        ...
        while buf.len() + frame::ResetStream::SIZE_BOUND < max_size {
            ...
            #[expect(
                clippy::expect_used,
                reason = "stream offsets never exceed the connection flow-control limit, itself a varint below 2^62"
            )]
            frame::ResetStream {
                id,
                error_code,
        ...
        for dir in Dir::iter() {
            ...
            // A raised limit (MAX_STREAMS) may have arrived while a lost STREAMS_BLOCKED waited
            // for retransmission; only announce a limit that still blocks the application.
    ...
    /// Whether a locally blocked `open` still has to be announced with STREAMS_BLOCKED
    pub(crate) fn has_streams_blocked(&self) -> bool {
    ...
    pub(crate) fn received_ack_of(&mut self, frame: frame::StreamMeta) {
        ...
        let id = frame.id;
        // What an acknowledgment releases is decided by the buffer: bytes acknowledged ahead of
        // the front are still held by their allocation until the front catches up.
    ...
    pub(crate) fn received_max_streams(
        ...
        if count > *current {
            *current = count;
            self.streams_blocked[dir as usize] = false;
            self.blocked_open[dir as usize] = false;
            self.events.push_back(StreamEvent::Available { dir });
        }
    ...
    pub(crate) fn queue_max_stream_id(&self, pending: &mut Retransmits) -> bool {
...
#[expect(
    clippy::unwrap_used,
    reason = "the closure replaces `Free` with `Open` and inserts `Open` when absent, so an `Open` entry is present"
)]
...
mod tests {
    ...
    #[test]
    fn initial_transport_limits_wake_blocked_opens() {
        let mut state = make(Side::Server);
        let established = crate::proto::connection::State::Established;
        for dir in Dir::iter() {
            assert!(
                (Streams {
                    state: &mut state,
        ...
        // Serializing STREAMS_BLOCKED must not erase the application's wait state.
        state.streams_blocked = [false, false];
        state.set_params(&TransportParameters {
            initial_max_streams_uni: 1u32.into(),
    ...
    fn reset_flow_control() {
        ...
        assert_eq!(
            ...
            crate::proto::ReadError::Reset(0u32.into())
        );
        let _transmit = chunks.finalize();
        assert_eq!(client.data_recvd, 4096);
        assert_eq!(client.local_max_data - initial_max, 4096);
    ...
    /// Stopping a stream returns the credit for everything received on it; the reset that
    /// follows returns only what lies beyond that, so the connection's receive window grows by
    /// exactly the final size and not by the stopped bytes a second time.
    #[test]
    fn stopped_reset() {
        for final_offset in [32u32, 48] {
            ...
            assert_eq!(
                ...
                "final size {final_offset}: the stopped bytes are not credited twice"
            );
        }
    }

    /// The send window bounds memory, so it counts what the buffers hold: data acknowledged
    /// ahead of the front is still held by its allocation until the front catches up, and a
    /// peer acknowledging selectively cannot open the window past what is retained.
    #[test]
    ...
    fn shrink_receive_window() {
        ...
        let credits = octets::kib_u64(512);
        let prev_local_max_data = server.local_max_data;
