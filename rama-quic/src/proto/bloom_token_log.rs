...
use crate::proto::{Duration, SystemTime, TokenLog, TokenReuseError, UNIX_EPOCH};
...
impl BloomTokenLog {
    ...
    pub(crate) fn new_expected_items(max_bytes: usize, expected_hits: u64) -> Self {
    ...
    pub(crate) fn new(max_bytes: usize, k_num: u32) -> Self {
...
/// Exact set of already-hashed nonces
///
/// The nonces are uniformly distributed 64-bit hashes, so an identity hasher is correct and
/// avoids hashing twice; `ahash` would add nothing here.
#[expect(
    clippy::disallowed_types,
    reason = "identity hasher over uniformly distributed nonce hashes"
)]
type IdentitySet = std::collections::HashSet<u64, IdentityBuildHasher>;
...
mod test {
    ...
    fn bloom_token_log_conversion() {
        ...
        for i in 0..200 {
            ...
            {
                let filter = &log.0.lock().filter_1;
        ...
        assert!(
            matches!(log.0.get_mut().filter_1, Filter::Bloom { .. }),
            "didn't bloom"
        );
    }
