...
/// Rama opens the connection, quiche answers it: a unidirectional stream up and a bidirectional
/// exchange, both verified by digest, both ended with FIN, and a bounded close on each side.
#[tokio::test]
async fn rama_client_to_quiche_server() {
    ...
    let (up_hash, question_hash, answer_hash) = (digest(&up), digest(&question), digest(&answer));
    ...
    let peer = Peer::spawn({
        ...
        async move {
            ...
            // The client opens the unidirectional stream first, then the bidirectional one.
            ...
            let asked = server.read_stream(0, octets::mib(1), deadline).await;
            assert_eq!(digest(&asked), question_hash, "the question arrived whole");
            server.write_stream(0, &answer, deadline).await;
            server
    ...
    let heard = deadline
        .wait("the answer", recv.read_to_end(octets::mib(1)))
        .await
        .expect("it completes");
    assert_eq!(digest(&heard), answer_hash, "the answer arrived whole");
...
/// quiche opens the connection, Rama answers it, with the same traffic in the same shapes.
#[tokio::test]
async fn quiche_client_to_rama_server() {
    let deadline = Deadline::new();
    let identity = Identity::generate("localhost");
    let server = deadline
        .wait(
    ...
    let up = payload(0x41, octets::kib(16));
    let question = payload(0x42, octets::kib(4));
    let answer = payload(0x43, octets::kib(8));
    let (up_hash, question_hash, answer_hash) = (digest(&up), digest(&question), digest(&answer));
    ...
    let served = Peer::spawn({
        ...
        async move {
            let incoming = deadline
                .wait("the rama server takes the attempt", server.accept())
                .await
            ...
            let received = deadline
                .wait("reading the uni stream", uni.read_to_end(octets::mib(1)))
                .await
                .expect("it completes");
            assert_eq!(digest(&received), up_hash, "the uni payload arrived whole");
            ...
            let (mut send, mut recv) = deadline
                ...
                .expect("it opens");
            let asked = deadline
                .wait("reading the question", recv.read_to_end(octets::mib(1)))
                .await
                .expect("it completes");
            assert_eq!(digest(&asked), question_hash, "the question arrived whole");
            deadline
                .wait("answering", send.write_all(&answer))
                .await
    ...
    peer.write_stream(2, &up, deadline).await;
    peer.write_stream(0, &question, deadline).await;
    let heard = peer.read_stream(0, octets::mib(1), deadline).await;
    assert_eq!(digest(&heard), answer_hash, "the answer arrived whole");
