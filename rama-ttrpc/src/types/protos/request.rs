...
`payload` is generic over our own `ProstField` trait, not `prost::Message`. That is what lets a
`Request` carry either a concrete typed message (client side) or an opaque, already-serialized
`RawBytes` blob that the server can route/forward without decoding. The derive macro only knows how
to encode a `#[prost(message)]` field whose type implements `prost::Message`, so it cannot dispatch
through `ProstField::{encode,encoded_len,merge}` — hence the manual impl below.

It is a modified version of the code generated by `cargo expand` on the following, with the
`payload` field re-pointed at `ProstField` instead of `prost::Message`:
```
