...
fn str_from_utf8(input: &[u8]) -> Result<&str, usize> {
    match decode(input) {
        Ok(s) => Ok(s),
        Err(
            DecodeError::Invalid { valid_prefix, .. }
            | DecodeError::Incomplete { valid_prefix, .. },
        ) => Err(valid_prefix.len()),
    }
}
...
fn test_str_from_utf8() {
    ...
    let xs = b"hello\xFF";
    str_from_utf8(xs).unwrap_err();
}

#[test]
fn test_is_utf8() {
    // Chars of 1, 2, 3, and 4 bytes
    str_from_utf8("eé€\u{10000}".as_bytes()).unwrap();
    // invalid prefix
    str_from_utf8(&[0x80]).unwrap_err();
    // invalid 2 byte prefix
    str_from_utf8(&[0xc0]).unwrap_err();
    str_from_utf8(&[0xc0, 0x10]).unwrap_err();
    // invalid 3 byte prefix
    str_from_utf8(&[0xe0]).unwrap_err();
    str_from_utf8(&[0xe0, 0x10]).unwrap_err();
    str_from_utf8(&[0xe0, 0xff, 0x10]).unwrap_err();
    // invalid 4 byte prefix
    str_from_utf8(&[0xf0]).unwrap_err();
    str_from_utf8(&[0xf0, 0x10]).unwrap_err();
    str_from_utf8(&[0xf0, 0xff, 0x10]).unwrap_err();
    str_from_utf8(&[0xf0, 0xff, 0xff, 0x10]).unwrap_err();

    // deny overlong encodings
    str_from_utf8(&[0xc0, 0x80]).unwrap_err();
    str_from_utf8(&[0xc0, 0xae]).unwrap_err();
    str_from_utf8(&[0xe0, 0x80, 0x80]).unwrap_err();
    str_from_utf8(&[0xe0, 0x80, 0xaf]).unwrap_err();
    str_from_utf8(&[0xe0, 0x81, 0x81]).unwrap_err();
    str_from_utf8(&[0xf0, 0x82, 0x82, 0xac]).unwrap_err();
    str_from_utf8(&[0xf4, 0x90, 0x80, 0x80]).unwrap_err();

    // deny surrogates
    str_from_utf8(&[0xED, 0xA0, 0x80]).unwrap_err();
    str_from_utf8(&[0xED, 0xBF, 0xBF]).unwrap_err();

    str_from_utf8(&[0xC2, 0x80]).unwrap();
    str_from_utf8(&[0xDF, 0xBF]).unwrap();
    str_from_utf8(&[0xE0, 0xA0, 0x80]).unwrap();
    str_from_utf8(&[0xED, 0x9F, 0xBF]).unwrap();
    str_from_utf8(&[0xEE, 0x80, 0x80]).unwrap();
    str_from_utf8(&[0xEF, 0xBF, 0xBF]).unwrap();
    str_from_utf8(&[0xF0, 0x90, 0x80, 0x80]).unwrap();
    str_from_utf8(&[0xF4, 0x8F, 0xBF, 0xBF]).unwrap();
}

/// A re-implementation of String::from_utf8_lossy
fn string_from_utf8_lossy(input: &[u8]) -> Cow<'_, str> {
...
fn all_partitions<F>(input: &[u8], f: F)
where
    F: Fn(&[&[u8]]),
{
...
impl io::Read for Chunks<'_> {
    #[expect(
        ...
        reason = "test stub: only the BufRead path is exercised, plain Read is never called"
    )]
