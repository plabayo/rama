#![allow(
    ...
    reason = "the entire point of these tests is to exercise `PartialEq<&str>`/`PartialEq<String>`/etc. on ArcStr; older rustc versions emitted `clippy::cmp_owned`, newer versions don't, so `#[expect]` can warn unfulfilled"
)]
