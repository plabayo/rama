...
impl Iterator for AckIter<'_> {
    ...
    fn next(&mut self) -> Option<RangeInclusive<u64>> {
        ...
        #[expect(
            clippy::unwrap_used,
            reason = "`AckIter` is built from a frame whose ranges `Iter::try_next` already decoded once via `scan_ack_blocks`"
        )]
...
mod test {
    ...
    fn immediate_ack_coding() {
        ...
        match &frames[0] {
            Frame::ImmediateAck => {}
            other => panic!("assertion failed: `{other:?}` does not match `Frame::ImmediateAck`"),
        }
    }
}
