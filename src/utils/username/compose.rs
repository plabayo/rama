...
macro_rules! impl_username_label_writer_either {
    ($id:ident, $($param:ident),+ $(,)?) => {
        impl<const SEPARATOR: char, $($param),+> UsernameLabelWriter<SEPARATOR> for crate::utils::combinators::$id<$($param),+>
        where
            $(
            ...
            )+
        {
            fn write_labels(&self, composer: &mut Composer<SEPARATOR>) -> Result<(), ComposeError> {
                match self {
                    $(
                        crate::utils::combinators::$id::$param(writer) => {
                            writer.write_labels(composer)
                        }
                    )+
                }
...
crate::utils::combinators::impl_either!(impl_username_label_writer_either);
