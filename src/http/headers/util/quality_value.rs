...
impl<T: str::FromStr> str::FromStr for QualityValue<T> {
    type Err = crate::http::headers::Error;
    fn from_str(s: &str) -> Result<QualityValue<T>, crate::http::headers::Error> {
        ...
        if let (Some(first), Some(second), None) = (parts.next(), parts.next(), parts.next()) {
            ...
            if first.starts_with("q=") || first.starts_with("Q=") {
                ...
                match q_part.parse::<f32>() {
                    ...
                    Err(_) => return Err(crate::http::headers::Error::invalid()),
                }
            }
        }
        match raw_item.parse::<T>() {
            ...
            Err(_) => Err(crate::http::headers::Error::invalid()),
        }
    }
}
