...
const WRITE_CONTENTION_MAX_SEND_BUFFER: u32 = 8 * 1024 * 1024;
...
async fn write_contention_benchmark() -> Result<(), Box<dyn Error>> {
    ...
    builder.set_initial_window_size(WRITE_CONTENTION_WINDOW);
    builder.set_initial_connection_window_size(WRITE_CONTENTION_WINDOW);
    builder.set_max_frame_size(WRITE_CONTENTION_MAX_FRAME_SIZE);
    builder.set_max_concurrent_streams(config.streams as u32);
    builder.set_max_send_buffer_size(WRITE_CONTENTION_MAX_SEND_BUFFER);
...
fn main() {
    ...
    let bench = std::env::var("H2_BENCH").unwrap_or_else(|_| "all".to_owned());
