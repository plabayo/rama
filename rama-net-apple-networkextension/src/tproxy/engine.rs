...
impl TransparentProxyEngineBuilder {
    ...
    /// Set a custom [`TcpFlow`] [`Service`] factory.
    ///
    ...
    /// Set a custom [`UdpFlow`] [`Service`] factory.
    ///
    /// The factory is invoked when the engine starts, with a fresh shutdown-aware context.
    #[must_use]
...
impl TransparentProxyEngine {
    pub fn start(&self) -> Result<(), BoxError> {
        ...
        state.udp_service = Some(udp_service);
    ...
    pub fn stop(&self, reason: i32) {
        let (shutdown, stop_trigger) = {
            ...
            state.running = false;
            state.tcp_service = None;
            state.udp_service = None;
...
impl TransparentProxyTcpSession {
    ...
    pub fn cancel(&mut self) {
        ...
        self.client_tx = None;
        let _ = self.eof_tx.send(true);
        // First trigger graceful per-flow cancellation, then hard-abort as fallback.
...
async fn run_tcp_bridge(
    ...
    // Arm one initial read demand so ingress EOF/teardown is detected promptly
    // even before any Rust-side poll on the TCP flow.
...
mod tests {
    ...
    fn tcp_bridge_requests_client_read_demand() {
        ...
        // Unblock the service-side read.
        _session.on_client_bytes(b"x");
    ...
    fn udp_session_requests_client_read_demand() {
        ...
        _session.on_client_datagram(b"x");
