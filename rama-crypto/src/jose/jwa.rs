...
impl TryFrom<JWA> for Box<&dyn RsaEncoding> {
    ...
    fn try_from(value: JWA) -> Result<Self, Self::Error> {
        match value {
            JWA::RS256 => Ok(Box::new(&RSA_PKCS1_SHA256)),
            JWA::RS384 => Ok(Box::new(&RSA_PKCS1_SHA384)),
            JWA::RS512 => Ok(Box::new(&RSA_PKCS1_SHA512)),
            JWA::PS256 => Ok(Box::new(&RSA_PSS_SHA256)),
            JWA::PS384 => Ok(Box::new(&RSA_PSS_SHA384)),
            JWA::PS512 => Ok(Box::new(&RSA_PSS_SHA512)),
            _ => Err(OpaqueError::from_display(
