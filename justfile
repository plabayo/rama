...
_zigbuild-linux-gnu-with-native-env-windows RECIPE TARGET:
    $rustVersion = (Select-String -Path rust-toolchain.toml -Pattern '^channel = "([^"]+)"$').Matches[0].Groups[1].Value; $env:RUSTUP_TOOLCHAIN = "${rustVersion}-x86_64-pc-windows-gnu"; $env:AR = "zig ar"; $env:AWS_LC_SYS_CMAKE_BUILDER = "0"; just {{RECIPE}} {{TARGET}}
