...
use rama_core::{
    bytes::{self, BufMut, Bytes, BytesMut},
    error::BoxError,
};
use rama_http_types::{
    Body, Response,
    ...
    E: Into<BoxError>,
{
    fn into_response(self) -> Response {
        (
            [
                (header::CONTENT_TYPE, mime::TEXT_EVENT_STREAM.as_ref()),
...
impl Event {
    ...
    ///
    /// [`MessageEvent`'s data field]: https://developer.mozilla.org/en-US/docs/Web/API/MessageEvent/data
    pub fn json_data<T>(mut self, data: T) -> Result<Event, BoxError>
    where
        T: serde::Serialize,
    {
...
mod tests {
    ...
    async fn basic() {
        ...
        let event_fields = parse_event(
            &String::from_utf8(stream.chunk().await.unwrap().unwrap().to_vec()).unwrap(),
        );
