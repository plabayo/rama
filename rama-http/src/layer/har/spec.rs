...
mod jiff_serializer {
    ...
    pub(super) fn deserialize<'de, D>(d: D) -> Result<Timestamp, D::Error>
    where
        D: Deserializer<'de>,
    {
        let s = <Cow<'de, str>>::deserialize(d)?;
        Timestamp::from_str(&s).map_err(Error::custom)
    }
}
...
pub struct Page {
    ...
    #[serde(with = "jiff_serializer", rename = "startedDateTime")]
