...
mod tests {
    ...
    async fn matcher_policy_empty() {
        ...
        for i in 0..10 {
            assert_ready(
                policy
                    .check(Context::default(), NumberedRequest::new(i))
                    .await,
            );
        }
    }
    ...
    async fn matcher_policy_always() {
        ...
        let guard_1 = assert_ready(policy.check(Context::default(), Extensions::new()).await);
        let guard_2 = assert_ready(policy.check(Context::default(), Extensions::new()).await);

        assert_abort(&policy.check(Context::default(), Extensions::new()).await);

        drop(guard_1);
        let _guard_3 = assert_ready(policy.check(Context::default(), Extensions::new()).await);

        assert_abort(&policy.check(Context::default(), Extensions::new()).await);

        drop(guard_2);
        assert_ready(policy.check(Context::default(), Extensions::new()).await);
    }
    ...
    async fn matcher_policy_scoped_limits() {
        ...
        for i in 1..10 {
            assert_ready(
                policy
                    .check(Context::default(), NumberedRequest::new(i * 2))
                    .await,
            );
        }

        let odd_guard_1 = assert_ready(
            policy
                .check(Context::default(), NumberedRequest::new(1))
                .await,
        );

        let const_guard_1 = assert_ready(
            policy
                .check(Context::default(), NumberedRequest::new(42))
                .await,
        );

        let odd_guard_2 = assert_ready(
            policy
                .check(Context::default(), NumberedRequest::new(3))
                .await,
        );
        ...
        assert_abort(
            &policy
                .check(Context::default(), NumberedRequest::new(5))
                .await,
        );
        assert_abort(
            &policy
        ...
        drop(odd_guard_1);
        let _odd_guard_3 = assert_ready(
            policy
                .check(Context::default(), NumberedRequest::new(9))
                .await,
        );
        ...
        assert_abort(
            &policy
                .check(Context::default(), NumberedRequest::new(11))
                .await,
        );
        ...
        assert_ready(
            policy
                .check(Context::default(), NumberedRequest::new(13))
                .await,
        );
