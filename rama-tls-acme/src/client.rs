...
use rama_crypto::{
    dep::rcgen::{self, Certificate},
    jose::{EMPTY_PAYLOAD, EcdsaKey, Empty, Headers, JWSBuilder, NO_PAYLOAD, Signer},
    pki_types::{CertificateDer, PrivatePkcs8KeyDer, pem::PemObject as _},
};
use rama_http::{
...
impl<'a> Order<'a> {
    ...
    pub async fn download_certificate_chain_no_checks(
        &self,
    ) -> Result<Vec<CertificateDer<'static>>, ClientError> {
        let bytes = self.download_certificate_no_checks().await?;

        let cert_chain = CertificateDer::pem_slice_iter(bytes.as_ref())
    ...
    pub async fn download_certificate_chain(
        ...
        self.wait_until_certificate_ready().await?;
        self.download_certificate_chain_no_checks().await
    }
    ...
    ) -> Result<(PrivatePkcs8KeyDer<'static>, Certificate), BoxError> {
