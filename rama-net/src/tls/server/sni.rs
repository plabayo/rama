...
mod test {
    ...
    async fn test_sni_router() {
        let tls_service = service_fn(async |_, req: SniRequest<_>| {
            let sni = req.sni.to_string();
            Ok::<_, Infallible>(sni)
        });
        let plain_service = service_fn(async |_, _| Ok::<_, Infallible>("plain".to_owned()));
        ...
        let response = peek_tls_svc
            ...
            .await
            .unwrap();
        assert_eq!("one.one.one.one", response);
