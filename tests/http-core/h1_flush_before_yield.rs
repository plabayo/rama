...
use rama::http::{Request, Response, StreamingBody, body::Frame};
...
/// whose readiness changes between the two write polls of the same
...
impl StreamingBody for PendOnceThenEnd {
    type Data = Bytes;
...
async fn h1_server_flushes_end_of_body_buffered_by_write_recheck() {
    ...
    tokio::spawn(async move {
        ...
        _ = http1::Builder::new()
            .serve_connection(ServiceInput::new(socket), RamaHttpService::new(service))
            .await;
    });
