...
mod conn {
            ...
            loop {
                tokio::select! {
                    res = listener.accept() => {
                        ...
                        tokio::task::spawn(async move {
                            let mut conn = http2::Builder::new(Executor::new())
    ...
    struct CountingStream {
        tcp: ServiceInput<TcpStream>,
        flush_count: Arc<AtomicUsize>,
    }

    impl ExtensionsRef for CountingStream {
        fn extensions(&self) -> &Extensions {
            self.tcp.extensions()
        }
    }
    ...
    async fn http1_half_closed_peer_with_open_request_body_does_not_spin() {
        ...
        let io = CountingStream {
            tcp: ServiceInput::new(tcp_connect(&addr).await.unwrap()),
