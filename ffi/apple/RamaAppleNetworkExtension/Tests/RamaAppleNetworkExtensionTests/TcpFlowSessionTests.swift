...
final class TcpFlowSessionTests: XCTestCase {
    ...
    /// handler can't drift from production. At the shipped default (`0`)
    /// the kill switch schedules nothing; enabled, a `false` schedules the
    /// coalesced re-check. The long settle keeps the timer from firing
    /// in-test (the fixture dies first; the weak-ctx guard then no-ops).
    func testViabilityHandlerSchedulesMidSessionRecheck() {
        ...
        fx.conn.simulateViability(false)
        XCTAssertFalse(
            fx.session.ctx.deadPathRecheckPending,
            "kill switch (shipped default 0) must schedule nothing")
