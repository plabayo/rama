...
- [`BoringClientConfigExt`](src/client/config.rs) on `TlsClientConfig`: `mimic_client_hello`, plus explicit `cipher_suites`, `supported_groups`, `signature_schemes`, `grease`, `alps`, `extension_order`, `permute_extensions`, `cert_compression`, `delegated_credentials`, `record_size_limit`, `encrypted_client_hello`, `ocsp_stapling`, `signed_cert_timestamps`, a custom server-verify `server_verify_cert_store` (`Arc<X509Store>`), and explicit `min_version` / `max_version`.
...
| **Server identity / SNI** ([RFC 6066](https://datatracker.ietf.org/doc/html/rfc6066)) | Resolved per-request from the target authority. DNS identities are verified and sent as SNI; IP identities are verified against `iPAddress` SANs and are not sent as SNI. Explicit `TlsServerName` and (tunnel mode) `TlsTunnel.server_identity` take precedence. | [connector.rs#L294-L401](src/client/connector.rs#L294-L401) |
...
| **Extension permutation** | `set_permute_extensions(bool)`, default false. BoringSSL generates a fresh permutation for each handshake, even with a reused context. A nonempty explicit order takes precedence: clear it with `set_extension_order(Vec::new())` to permute a mimicked ClientHello. Native GREASE and PSK placement rules still apply. | [config.rs](src/client/config.rs), [connector_data.rs](src/client/connector_data.rs) |
...
| **Server verification** | `ServerVerifyMode::Auto` (default) requires an identity and verifies both the chain and that identity; `Disable` installs a `NONE` verify callback that always accepts. | [connector_data.rs#L317-L325](src/client/connector_data.rs#L317-L325) |
| **Trust store** | Common policy supports native roots (webpki/CCADB fallback), explicit bundled WebPKI roots, custom-only roots, and additional private roots with the same semantics as rustls. Stores for the exact native and WebPKI sets are cached process-wide. A custom `Arc<X509Store>` overrides the common policy. | [connector_data.rs](src/client/connector_data.rs) |
...
| **Connector kinds** | `auto` (handshake only for secure protocols, default), `secure` (always), `tunnel(identity)` (handshake when `TlsTunnel` is present or a hardcoded identity is configured). | [connector.rs#L183-L329](src/client/connector.rs#L183-L329) |
...
| **Identity-aware cert selection** | `select_certificate` (in-memory CA) / async `select_certificate` (dynamic issuer) resolve a DNS identity from SNI, otherwise use the target or configured fallback identity; IP identities therefore work without IP literals in SNI. A static `ServerAuthData` sets a fixed cert up front. | [acceptor_data.rs](src/server/acceptor_data.rs) |
...
| **Cert source: generated** | `GeneratedServerAuthConfig` creates either a directly self-signed leaf or a generated self-signed CA plus leaf. DNS and IP SANs, key kinds, validity and distinguished names are configured through the shared rama TLS certificate types. Serial numbers are generated automatically. The resulting `ServerAuthData` is stable across connections. | [rama-crypto cert](../rama-crypto/src/cert/) |
| **Cert source: issuer (on-the-fly)** | `ServerCertIssuerKind` issues per-identity leaves from a generated CA, a provided CA chain+key, or a user `DynamicCertIssuer`. Generated and parsed CA material is shared across connections. | [cert_issuer.rs](src/server/cert_issuer.rs), [acceptor_data.rs](src/server/acceptor_data.rs) |
| **Issued-cert caching** | A shared moka `Cache<CertificateIdentity, IssuedCert>` supports DNS and IP identities; `Disabled` or `MemCache { max_size, ttl }`. The default cache has no TTL, while expired or not-yet-valid leaves are always rejected and reissued. | [cert_issuer.rs](src/server/cert_issuer.rs), [acceptor_data.rs](src/server/acceptor_data.rs) |
| **mTLS / client certificate verification** | `ClientVerifyMode::ClientAuth` advertises acceptable client-CA names via `add_client_ca` and enables `SSL_VERIFY_PEER | SSL_VERIFY_FAIL_IF_NO_PEER_CERT` to require and verify client certificates. The OS trust store is deliberately not loaded; only the configured client CAs are trusted. | [service.rs#L115-L130](src/server/service.rs#L115-L130), [acceptor_data.rs#L269-L292](src/server/acceptor_data.rs#L269-L292) |
...
| Fallback | Mirror → default-with-SNI (no ALPN), with the downgrade logged. Connector-config errors are surfaced. The bare `BridgeIo` path (no captured hello) ships `rama-boring` defaults with verification explicitly disabled and a warning. | [service.rs#L82-L162](src/proxy/mitm/service.rs#L82-L162) |
