...
> **Backend scope.** Within the rama ecosystem these extended capabilities — especially TLS **mirroring**, (user-agent) **emulation** and **fingerprinting** — are for now only available in `rama-tls-boring`, not [`rama-tls-rustls`](../rama-tls-rustls/). We eventually plan to fork [`rustls`](https://github.com/rustls/rustls) and adapt it to our needs (kept bi-directionally in sync with upstream where possible), so we can step away from the C/C++ of boringssl for the resulting safety and performance gains — most likely prioritised through a paid partner feature request.
>
...
| **Cipher suites** | Ordered, GREASE/unknown-preserving raw u16 list via `set_raw_cipher_list`. `CipherSuite` is an open u16 enum. | [connector_data.rs#L103-L105](src/client/connector_data.rs#L103-L105), [connector_data.rs#L236-L241](src/client/connector_data.rs#L236-L241) |
...
| **Extension order** | Explicit `set_extension_order` from `ExtensionId` list (no random permutation). | [connector_data.rs#L106-L108](src/client/connector_data.rs#L106-L108), [connector_data.rs#L229-L234](src/client/connector_data.rs#L229-L234) |
...
| **mTLS / client auth** | `ClientAuth::SelfSigned` (fresh 4096-bit RSA self-signed) or `Single` (supplied chain+key in DER/DER-stack/PEM) via `set_private_key`/`set_certificate`/`add_extra_chain_cert`. | [connector_data.rs#L327-L351](src/client/connector_data.rs#L327-L351), [connector_data.rs#L470-L597](src/client/connector_data.rs#L470-L597) |
...
The inbound acceptor ([`TlsAcceptorLayer`](src/server/layer.rs) / `TlsAcceptorService` / [`TlsAcceptorData`](src/server/acceptor_data.rs)) builds a fresh `SslAcceptor` per connection.
...
| **Protocol versions** | If `ServerConfig.protocol_versions` is set, min and max of the list applied as `set_min/max_proto_version`. Else the v5 profile governs. | [service.rs#L106-L122](src/server/service.rs#L106-L122) |
...
| **SNI-aware cert selection** | `select_certificate` (InMemoryIssuer) / async `select_certificate` (DynamicIssuer) resolve the domain from client SNI (else context `server_name`) and install the leaf+chain+key at handshake. Static `Single`/`SelfSigned` set a fixed cert up front (not per-SNI). | [acceptor_data.rs#L131-L258](src/server/acceptor_data.rs#L131-L258), [acceptor_data.rs#L369-L490](src/server/acceptor_data.rs#L369-L490) |
| **Cert source: static** | `ServerAuth::Single` (supplied chain+key, DER/DER-stack/PEM) or `SelfSigned` (in-memory self-signed CA+leaf); `check_private_key` validates the pair. | [acceptor_data.rs#L294-L432](src/server/acceptor_data.rs#L294-L432) |
| **Cert source: self-signed gen** | CA (20-year, BasicConstraints CA, keyCertSign+cRLSign) + leaf (90-day end-entity, SAN=CN), 159-bit random serials. Key kinds RSA2048/4096, EC P-256/384/521, Ed25519 (default Ec P-256). | [acceptor_data.rs#L492-L502](src/server/acceptor_data.rs#L492-L502), [utils/certs.rs#L161-L290](src/server/utils/certs.rs#L161-L290) |
| **Cert source: issuer (on-the-fly)** | `ServerAuth::CertIssuer` mints a per-SNI leaf signed by an in-memory CA (`SelfSigned`/`Single`) or a user `Dynamic` async issuer. | [acceptor_data.rs#L307-L490](src/server/acceptor_data.rs#L307-L490) |
...
| **Key logging** | `set_keylog_callback` from `ServerConfig.key_logger` (`Environment`/`Disabled`/`File`/`Custom`); default `Environment`. | [service.rs#L163-L170](src/server/service.rs#L163-L170) |
...
| **Per-connection override** | Inject a `TlsAcceptorData` into the inbound stream extensions to override the layer's `.config` for that connection (e.g. ACME-renewed certs). | [service.rs#L58-L67](src/server/service.rs#L58-L67) |
...
**Utilities (exported, not wired into the acceptor service):** an OCSP "good" staple builder (`build_mitm_leaf_ocsp_response`, [RFC 6066](https://datatracker.ietf.org/doc/html/rfc6066)) and a cert-mirroring re-signer (`self_signed_server_auth_mirror_cert`) live in [server::utils](src/server/utils/) and are consumed by the MITM proxy below — the acceptor itself staples nothing.
...
| Max-version clamp | Egress max version clamped to TLS 1.2 when the mirrored hello is not a viable TLS 1.3 offer (no supported_versions, or advertises 1.3 without a 1.3 cipher/sigalg). | [config.rs#L394-L442](src/client/config.rs#L394-L442) |
...
**Certificate mirroring / forging** ([`self_signed_server_auth_mirror_cert`](src/server/utils/certs.rs#L296-L447)) — the re-signed leaf copies the upstream subject name, notBefore/notAfter, and most extensions verbatim (preserving criticality), with a **fresh** key matched to the source key type (RSA→RSA max(bits,2048); EC on the source group; Ed25519; else RSA-2048 fallback). SKI/AKI are regenerated (only when the source carried them). Stripped: CRL Distribution Points, Authority Information Access, Freshest CRL, RFC 7633 must-staple, and RFC 6962 embedded SCTs — extensions invalid or unsatisfiable after re-signing.
...
| --- | --- | --- |
| `InMemoryBoringMitmCertIssuer` | Mirrors + re-signs each upstream leaf with an in-memory CA; attaches a best-effort OCSP staple. | [issuer/memory.rs](src/proxy/mitm/issuer/memory.rs) |
...
- Backend-agnostic config: **`KeyLogIntent`**, **`ServerVerifyMode`**, **`ClientVerifyMode`** / **`ClientAuth`**, **`DataEncoding`** (DER / DER-stack / PEM, with redacting `Debug`), and the **`ServerAuth`** / **`SelfSignedData`** / **`ServerCertIssuer*`** family.
