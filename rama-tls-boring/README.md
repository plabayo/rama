...
- Backend-agnostic config: **`KeyLogIntent`**, **`ServerVerifyMode`**, **`ClientVerifyMode`** / **`ClientAuth`**, **`DataEncoding`** (DER / DER-stack / PEM, with redacting `Debug`), and the **`ServerAuthData`** (concrete DER cert material) / **`ServerCertIssuer*`** family. Self-signed generation (**`SelfSignedData`** + `self_signed_server_auth`) lives in [`rama-crypto`](../rama-crypto/src/cert/), backend-pluggable across boring / aws-lc / ring.
