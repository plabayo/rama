...
| **Cert source: generated** | `GeneratedServerAuthConfig` creates either a directly self-signed leaf or a generated self-signed CA plus leaf. DNS and IP SANs, key kinds, validity, serial numbers and distinguished names are configured through the shared rama TLS certificate types. The resulting `ServerAuthData` is stable across connections. | [rama-crypto cert](../rama-crypto/src/cert/) |
