...
impl FlatCsvSeparator {
    fn as_byte(self) -> u8 {
        match self {
            FlatCsvSeparator::Comma => b',',
            FlatCsvSeparator::SemiColon => b';',
        }
    }
...
where
    T: TryFrom<&'a str, Error: Into<BoxError>>,
{
    let mut in_quotes = false;
...
pub(crate) fn try_encode_non_empty_vec_as_flat_csv_header_value<'a, T>(
