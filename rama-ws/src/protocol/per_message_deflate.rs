    ...
    Fn: FnOnce(&[u8], &mut [u8]) -> (Result<Status, E>, u64),
{
    ...
    #[expect(clippy::multiple_unsafe_ops_per_block, reason = "single uninitialized-tail write sequence: ptr.add → from_raw_parts_mut → set_len; splitting them harms readability without changing the safety contract")]
    unsafe {
