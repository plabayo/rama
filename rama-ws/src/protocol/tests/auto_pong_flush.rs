...
fn read_usage_auto_pong_flush() {
    let mut ws = WebSocket::from_raw_socket(MockWrite::default(), Role::Client, None);
    ...
    assert!(
        ws.get_ref().flushed_data.is_empty(),
        "Unexpected {:?}",
        ws.get_ref()
    );
    ...
    let pong_header = FrameHeader::parse(&mut Cursor::new(&ws.get_ref().written_data))
        .unwrap()
        .unwrap()
        .0;
    assert_eq!(pong_header.opcode, OpCode::Control(OpCodeControl::Pong));
    ...
    assert!(
        ws.get_ref().flushed_data == written_data,
        "Unexpected {:?}",
        ws.get_ref()
    );
